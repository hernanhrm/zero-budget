...
func (r postgres) FindOne(ctx context.Context, criteria dafi.Criteria) (domain.EmailLog, error) {
	...
	if err != nil {
		return domain.EmailLog{}, oops.WithContext(ctx).In(apperrors.LayerRepository).Wrap(err)
	}
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return domain.EmailLog{}, oops.WithContext(ctx).In(apperrors.LayerRepository).Code(apperrors.CodeNotFound).Wrap(err)
		}
		return domain.EmailLog{}, oops.WithContext(ctx).In(apperrors.LayerRepository).Wrap(err)
	}
...
func (r postgres) FindAll(ctx context.Context, criteria dafi.Criteria) (basedomain.List[domain.EmailLog], error) {
	query := sqlcraft.Select(columns...).
		From(tableName).
		Where(criteria.Filters...).
	...
	var logs basedomain.List[domain.EmailLog]
	for rows.Next() {
		var log domain.EmailLog
		err = rows.Scan(
			&log.ID,
...
func (r postgres) Create(ctx context.Context, input domain.CreateEmailLog) error {
	now := time.Now()
...
func (r postgres) CreateBulk(ctx context.Context, inputs basedomain.List[domain.CreateEmailLog]) error {
	if inputs.IsEmpty() {
		return nil
	}
...
func (r postgres) Update(ctx context.Context, input domain.UpdateEmailLog, filters ...dafi.Filter) error {
