...
import (
	...
	"backend/core/auth/organization/port"
...
func (r postgres) FindOne(ctx context.Context, criteria dafi.Criteria) (port.Organization, error) {
	...
	if err != nil {
		return port.Organization{}, oops.WithContext(ctx).In(apperrors.LayerRepository).Wrap(err)
	}
	...
	var organization port.Organization
	err = row.Scan(
		&organization.ID,
		&organization.Name,
		&organization.Slug,
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return port.Organization{}, oops.WithContext(ctx).In(apperrors.LayerRepository).Code(apperrors.CodeNotFound).Wrap(err)
		}
		return port.Organization{}, oops.WithContext(ctx).In(apperrors.LayerRepository).Wrap(err)
	}
...
func (r postgres) FindAll(ctx context.Context, criteria dafi.Criteria) (basedomain.List[port.Organization], error) {
	query := sqlcraft.Select(columns...).
		From(tableName).
		Where(criteria.Filters...).
	...
	var organizations basedomain.List[port.Organization]
	for rows.Next() {
		var organization port.Organization
		err = rows.Scan(
			&organization.ID,
...
func (r postgres) Create(ctx context.Context, input port.CreateOrganization) error {
	now := time.Now()
...
func (r postgres) CreateBulk(ctx context.Context, inputs basedomain.List[port.CreateOrganization]) error {
	if inputs.IsEmpty() {
		return nil
	}
...
func (r postgres) Update(ctx context.Context, input port.UpdateOrganization, filters ...dafi.Filter) error {
