...
func (r postgres) FindOne(ctx context.Context, criteria dafi.Criteria) (port.Role, error) {
	...
	if err != nil {
		return port.Role{}, oops.WithContext(ctx).In(apperrors.LayerRepository).Wrap(err)
	}
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return port.Role{}, oops.WithContext(ctx).In(apperrors.LayerRepository).Code(apperrors.CodeNotFound).Wrap(err)
		}
		return port.Role{}, oops.WithContext(ctx).In(apperrors.LayerRepository).Wrap(err)
	}
...
func (r postgres) FindAll(ctx context.Context, criteria dafi.Criteria) (basedomain.List[port.Role], error) {
	query := sqlcraft.Select(columns...).
		From(tableName).
		Where(criteria.Filters...).
	...
	var roles basedomain.List[port.Role]
	for rows.Next() {
		var role port.Role
		err = rows.Scan(
			&role.ID,
...
func (r postgres) Create(ctx context.Context, input port.CreateRole) error {
	now := time.Now()
...
func (r postgres) CreateBulk(ctx context.Context, inputs basedomain.List[port.CreateRole]) error {
	if inputs.IsEmpty() {
		return nil
	}
...
func (r postgres) Update(ctx context.Context, input port.UpdateRole, filters ...dafi.Filter) error {
