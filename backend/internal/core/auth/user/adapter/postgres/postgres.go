...
func (r postgres) FindOne(ctx context.Context, criteria dafi.Criteria) (port.User, error) {
	...
	if err != nil {
		return port.User{}, oops.WithContext(ctx).In(apperrors.LayerRepository).Wrap(err)
	}
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return port.User{}, oops.WithContext(ctx).In(apperrors.LayerRepository).Code(apperrors.CodeNotFound).Wrap(err)
		}
		return port.User{}, oops.WithContext(ctx).In(apperrors.LayerRepository).Wrap(err)
	}
...
func (r postgres) FindAll(ctx context.Context, criteria dafi.Criteria) (basedomain.List[port.User], error) {
	query := sqlcraft.Select(columns...).
		From(tableName).
		Where(criteria.Filters...).
	...
	var users basedomain.List[port.User]
	for rows.Next() {
		var user port.User
		err = rows.Scan(
			&user.ID,
...
func (r postgres) Create(ctx context.Context, input port.CreateUser) error {
	now := time.Now()

	query := sqlcraft.InsertInto(tableName).
		WithColumns(columns...).
		WithValues(input.ID, input.FirstName, input.LastName, input.Email, input.Password, nil, now, now)
...
func (r postgres) CreateBulk(ctx context.Context, inputs basedomain.List[port.CreateUser]) error {
	if inputs.IsEmpty() {
		return nil
	}
	...
	for _, input := range inputs {
		query = query.WithValues(input.ID, input.FirstName, input.LastName, input.Email, input.Password, nil, now, now)
	}
...
func (r postgres) Update(ctx context.Context, input port.UpdateUser, filters ...dafi.Filter) error {
