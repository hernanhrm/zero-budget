...
import (
	...
	"backend/core/auth/workspace/port"
...
func (r postgres) FindOne(ctx context.Context, criteria dafi.Criteria) (port.Workspace, error) {
	...
	if err != nil {
		return port.Workspace{}, oops.WithContext(ctx).In(apperrors.LayerRepository).Wrap(err)
	}
	...
	var item port.Workspace
	err = row.Scan(&item.ID, &item.OrganizationID, &item.Name, &item.Slug, &item.CreatedAt, &item.UpdatedAt)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return port.Workspace{}, oops.WithContext(ctx).In(apperrors.LayerRepository).Code(apperrors.CodeNotFound).Wrap(err)
		}
		return port.Workspace{}, oops.WithContext(ctx).In(apperrors.LayerRepository).Wrap(err)
	}
	return item, nil
...
func (r postgres) FindAll(ctx context.Context, criteria dafi.Criteria) (basedomain.List[port.Workspace], error) {
	query := sqlcraft.Select(columns...).
		From(tableName).
		Where(criteria.Filters...).
	...
	var items basedomain.List[port.Workspace]
	for rows.Next() {
		var item port.Workspace
...
func (r postgres) Create(ctx context.Context, input port.CreateWorkspace) error {
	now := time.Now()

	query := sqlcraft.InsertInto(tableName).
		WithColumns(columns...).
		WithValues(input.ID, input.OrganizationID, input.Name, input.Slug, now, now)
...
func (r postgres) CreateBulk(ctx context.Context, inputs basedomain.List[port.CreateWorkspace]) error {
	if inputs.IsEmpty() {
		return nil
	}
	now := time.Now()
	...
	for _, input := range inputs {
		query = query.WithValues(input.ID, input.OrganizationID, input.Name, input.Slug, now, now)
	}
...
func (r postgres) Update(ctx context.Context, input port.UpdateWorkspace, filters ...dafi.Filter) error {
