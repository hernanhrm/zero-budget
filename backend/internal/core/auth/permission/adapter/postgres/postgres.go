...
import (
	...
	"backend/core/auth/permission/port"
...
func (r postgres) FindOne(ctx context.Context, criteria dafi.Criteria) (port.Permission, error) {
	...
	if err != nil {
		return port.Permission{}, oops.WithContext(ctx).In(apperrors.LayerRepository).Wrap(err)
	}
	...
	var item port.Permission
	err = row.Scan(&item.ID, &item.Slug, &item.Description)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return port.Permission{}, oops.WithContext(ctx).In(apperrors.LayerRepository).Code(apperrors.CodeNotFound).Wrap(err)
		}
		return port.Permission{}, oops.WithContext(ctx).In(apperrors.LayerRepository).Wrap(err)
	}
	return item, nil
...
func (r postgres) FindAll(ctx context.Context, criteria dafi.Criteria) (basedomain.List[port.Permission], error) {
	query := sqlcraft.Select(columns...).
		From(tableName).
		Where(criteria.Filters...).
	...
	var items basedomain.List[port.Permission]
	for rows.Next() {
		var item port.Permission
...
func (r postgres) Create(ctx context.Context, input port.CreatePermission) error {
...
func (r postgres) CreateBulk(ctx context.Context, inputs basedomain.List[port.CreatePermission]) error {
	if inputs.IsEmpty() {
		return nil
	}
...
func (r postgres) Update(ctx context.Context, input port.UpdatePermission, filters ...dafi.Filter) error {
