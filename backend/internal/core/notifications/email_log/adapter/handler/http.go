...
import (
	"backend/core/notifications/email_log/port"
...
func NewHTTP(svc port.Service, logger basedomain.Logger) HTTP {
	return HTTP{
		svc:    svc,
		logger: logger.With("component", "email_log.handler"),
	}
}

func (h HTTP) FindByTemplate(c echo.Context) error {
	...
	criteria.Filters = append(criteria.Filters, dafi.FilterBy("templateId", dafi.Equal, templateID)...)

	logs, err := h.svc.FindAll(ctx, criteria)
	if err != nil {
		return oops.WithContext(ctx).In(apperrors.LayerHandler).Wrap(err)
	}

	return httpresponse.OK(c, logs)
}
...
func (h HTTP) FindOne(c echo.Context) error {
	...
	log, err := h.svc.FindOne(ctx, criteria)
	if err != nil {
		return oops.WithContext(ctx).In(apperrors.LayerHandler).Wrap(err)
	}

	return httpresponse.OK(c, log)
}
