...
func NewHTTP(svc port.Service, logger basedomain.Logger) HTTP {
	return HTTP{
		svc:    svc,
		logger: logger.With("component", "category.handler"),
	}
}
...
func (h HTTP) FindOne(c echo.Context) error {
	...
	cat, err := h.svc.FindOne(ctx, criteria)
	if err != nil {
		return oops.WithContext(ctx).In(apperrors.LayerHandler).Wrap(err)
	}

	return httpresponse.OK(c, cat)
}
...
func (h HTTP) FindAll(c echo.Context) error {
	...
	cats, err := h.svc.FindAll(ctx, criteria)
	if err != nil {
		return oops.WithContext(ctx).In(apperrors.LayerHandler).Wrap(err)
	}

	return httpresponse.OK(c, cats)
}

func (h HTTP) Create(c echo.Context) error {
	ctx := c.Request().Context()

	var input port.CreateCategory
