...
func (s service) FindOne(ctx context.Context, criteria dafi.Criteria) (domain.EmailLog, error) {
	log, err := s.repo.FindOne(ctx, criteria)
	if err != nil {
		return domain.EmailLog{}, oops.WithContext(ctx).In(apperrors.LayerService).Wrap(err)
	}

	return log, nil
}

func (s service) FindAll(ctx context.Context, criteria dafi.Criteria) (basedomain.List[domain.EmailLog], error) {
...
func (s service) Create(ctx context.Context, input domain.CreateEmailLog) error {
...
func (s service) CreateBulk(ctx context.Context, inputs basedomain.List[domain.CreateEmailLog]) error {
...
func (s service) Update(ctx context.Context, input domain.UpdateEmailLog, filters ...dafi.Filter) error {
	if err := input.Validate(ctx); err != nil {
		return oops.WithContext(ctx).In(apperrors.LayerService).Code(apperrors.CodeValidation).Wrap(err)
	}
