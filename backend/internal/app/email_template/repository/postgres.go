...
func (r postgres) FindOne(ctx context.Context, criteria dafi.Criteria) (domain.EmailTemplate, error) {
	...
	if err != nil {
		return domain.EmailTemplate{}, oops.WithContext(ctx).In(apperrors.LayerRepository).Wrap(err)
	}
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return domain.EmailTemplate{}, oops.WithContext(ctx).In(apperrors.LayerRepository).Code(apperrors.CodeNotFound).Wrap(err)
		}
		return domain.EmailTemplate{}, oops.WithContext(ctx).In(apperrors.LayerRepository).Wrap(err)
	}
...
func (r postgres) FindAll(ctx context.Context, criteria dafi.Criteria) (basedomain.List[domain.EmailTemplate], error) {
	query := sqlcraft.Select(columns...).
		From(tableName).
		Where(criteria.Filters...).
	...
	var tmpls basedomain.List[domain.EmailTemplate]
	for rows.Next() {
		var tmpl domain.EmailTemplate
		err = rows.Scan(
			&tmpl.ID,
...
func (r postgres) Create(ctx context.Context, input domain.CreateEmailTemplate) error {
	now := time.Now()
...
func (r postgres) CreateBulk(ctx context.Context, inputs basedomain.List[domain.CreateEmailTemplate]) error {
	if inputs.IsEmpty() {
		return nil
	}
...
func (r postgres) Update(ctx context.Context, input domain.UpdateEmailTemplate, filters ...dafi.Filter) error {
